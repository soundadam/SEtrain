...
class DPGRNN(nn.Module):
    ...
    def __init__(self, input_size, width, hidden_size, **kwargs):
        ...
        # if require online, set bidirectional=False
        self.inter_rnn = GRNN(input_size=input_size, hidden_size=hidden_size//2, bidirectional=True)
...
class Encoder(nn.Module):
    def __init__(self, channelsize=64, gt_blocks_repeat=3):
        super().__init__()
        self.channelsize = channelsize
        self.gt_blocks_repeat = gt_blocks_repeat
        self.en_convs = nn.ModuleList([
            ConvBlock(3*3, 24, (1,5), stride=(1,2), padding=(0,2), use_deconv=False, is_last=False),
            ConvBlock(24, self.channelsize, (1,5), stride=(1,2), padding=(0,2), groups=2, use_deconv=False, is_last=False),
        ])
        for i in range(self.gt_blocks_repeat):
            self.en_convs.append(
                GTConvBlock(self.channelsize, self.channelsize, (3,3), stride=(1,1),padding=(0, 1),dilation=(3**i, 1))  # 保持输出尺寸不变
            )
    def forward(self, x):
        en_outs = []
...
class Decoder(nn.Module):
    def __init__(self, channelsize=64, gt_blocks_repeat=3):
        ...
        # 镜像对称的dilation组合
        for i in reversed(range(self.gt_blocks_repeat)):
            dilation = 3 ** i
            self.de_convs.append(
                GTConvBlock(self.channelsize, self.channelsize, (3,3), stride=(1,1), padding=(2*dilation,1), dilation=(dilation, 1),use_deconv=True)
            )
        self.de_convs.extend([
            ConvBlock(self.channelsize, 24, (1,5), stride=(1,2), padding=(0,2), groups=2, use_deconv=True, is_last=False),
            ConvBlock(24, 2, (1,5), stride=(1,2), padding=(0,2), use_deconv=True, is_last=True)
        ])
...
class GTCRN(nn.Module):
    def __init__(
        ...
        self.encoder = Encoder(channelsize=self.channelsize, gt_blocks_repeat=self.gt_blocks_repeat)

        self.dpgrnn1 = DPGRNN(self.channelsize, 33, self.channelsize)
        self.dpgrnn2 = DPGRNN(self.channelsize, 33, self.channelsize)

        self.decoder = Decoder(channelsize=self.channelsize, gt_blocks_repeat=self.gt_blocks_repeat)
