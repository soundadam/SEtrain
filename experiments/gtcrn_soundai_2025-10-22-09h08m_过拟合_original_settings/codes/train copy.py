...
# from dataloader_dns3 import DNS3Dataset as Dataset  # Original DNS3 dataset (hardcoded paths)
# from dataloader_soundai_simple import DNS3Dataset as Dataset  # Old adapter (complex)
# from dataloader import SoundaiDatasetSimple as Dataset  # New simple dataloader (SCP-based, DNS3-compatible)
from dataloader_soundai import DNS3Dataset as Dataset
...
def run(rank, config, args):
    ...
    # ensure process group destroyed cleanly
    if args.world_size > 1:
        try:
            if dist.is_available() and dist.is_initialized():
                dist.destroy_process_group()
        except Exception as e:
            print(f"Warning: error while destroying process group on rank {rank}: {e}", file=sys.stderr)
...
class Trainer:
    ...
    def _save_checkpoint(self, epoch, score, save_regular=True):
        """
        Save checkpoint.

        Args:
            epoch: Current epoch
            score: Validation score (SI-SDR)
            save_regular: Whether to save regular checkpoint (default: True)
        """
        ...
        # Save regular checkpoint (only if requested, to save IO)
        if save_regular:
        ...
        if score > self.best_score:
            ...
            # Also save best model immediately
    ...
    def _validation_epoch(self, epoch):
        ...
        for step, (noisy, clean) in enumerate(self.validation_bar, 1):
            ...
            pesq_score_batch = Parallel(n_jobs=-1)(
                delayed(pesq)(16000, c, e, 'wb') for c, e in zip(clean_np, enhanced_np))
            ...
            self.validation_bar.postfix = 'loss={:.3f}, pesq={:.3f}, sisdr={:.2f}'.format(
                total_loss / step, total_pesq_score / step, total_sisdr_score / step)
        ...
        return avg_loss, avg_pesq, avg_sisdr
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs + self.start_epoch):
            ...
            self._set_eval_mode()
            valid_loss, valid_pesq, valid_sisdr = self._validation_epoch(epoch)

            # Use SI-SDR as the primary metric for model selection
            score = valid_sisdr
...
if __name__ == '__main__':
    ...
    parser.add_argument('-C', '--config', default='configs/cfg_train_soundai.yaml')
