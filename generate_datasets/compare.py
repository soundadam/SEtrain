...
class DatasetConfig:
    ...
    def __init__(self, args):
        ...
        self.target_dir = self.data_dir  # 修改：直接在数据目录下创建文件夹
...
class DatasetGenerator:
    ...
    def setup(self):
        ...
        # 创建 train_clean 和 train_noisy 目录
        makedirs(join(self.config.target_dir, "train_clean"), exist_ok=True)
        makedirs(join(self.config.target_dir, "train_noisy"), exist_ok=True)
        if self.config.save_reverberant:
            makedirs(join(self.config.target_dir, "train_reverberant"), exist_ok=True)
    ...
    def _load_rir_files(self):
        ...
        # ACE-Challenge (test)
        ace_dir = join(data_dir, "ACE-Challenge")
        if isdir(ace_dir):
        ...
        # Palimpsest (train)
        pal_dir = join(data_dir, "Palimpsest")
        if isdir(pal_dir):
            self.rir_files["train"] += sorted(glob(join(pal_dir, "**", "*.wav"), recursive=True))

        # davidscipka-MIT (train)
        david_dir = join(data_dir, "davidscipka-MIT_environmental_impulse_responses")
        if isdir(david_dir):
    ...
    def _write_csv_header(self, subset: str):
        ...
        with open(csv_path, "w") as f:
            f.write("id,speaker,speech_file,speech_start,speech_end,"
    ...
    def _get_speech_files(self, subset: str) -> List[str]:
        ...
        speech_files = []
        for speaker in os.listdir(self.config.speech_dir):
            speaker_dir = join(self.config.speech_dir, speaker)
            if isdir(speaker_dir):
                speech_files += sorted(glob(join(speaker_dir, "*.wav")))
    ...
    def _save_sample(self, subset: str, speech_file: str,
        ...
        """Save processed audio样本（修改后）"""
        speaker = basename(dirname(speech_file))
        sample_id = self.sample_ids[subset]
        self.sample_ids[subset] += 1

        if subset == "train":
            # 训练集使用新格式
            clean_path = join(self.config.target_dir, "train_clean", f"clean_{sample_id:05d}.wav")
            noisy_path = join(self.config.target_dir, "train_noisy", f"noisy_{sample_id:05d}.wav")
            
            # 保存音频
            write(noisy_path, mixture, self.config.target_sr, subtype="FLOAT")
            write(clean_path, speech, self.config.target_sr, subtype="FLOAT")
            
            if self.config.save_reverberant:
                rever_path = join(self.config.target_dir, "train_reverberant", f"reverberant_{sample_id:05d}.wav")
                write(rever_path, reverberant, self.config.target_sr, subtype="FLOAT")
        else:
            # 验证集和测试集保持原格式
            base_path = join(self.config.target_dir, subset)
            write(
                join(base_path, "noisy", speaker, f"{sample_id:05d}_{rir_info['rt60']:.2f}_{snr_dB:.1f}dB.wav"),
                mixture, self.config.target_sr, subtype="FLOAT"
            )
            write(
                join(base_path, "clean", speaker, f"{sample_id:05d}.wav"),
                speech, self.config.target_sr, subtype="FLOAT"
            )
            if self.config.save_reverberant:
                write(
                    join(base_path, "reverberant", speaker, f"{sample_id:05d}_{rir_info['rt60']:.2f}.wav"),
                    reverberant, self.config.target_sr, subtype="FLOAT"
                )
        ...
        with open(csv_path, "a") as f:
            f.write(
                f"{sample_id:05d},{speaker},{basename(speech_file)[:-4]},"
                f"{speech_start},{speech_end if speech_end != -1 else len(speech)},"
                f"{rir_info['file'].replace(self.config.data_dir, '')},{rir_info['channel']},"
                f"{rir_info['gain']},{rir_info['rt60']:.2f},"
