...
def convert_audio_file(args):
    flac_path, wav_path, target_sr, normalize = args
...
def convert_soundai_datasets(
    source_root='soundai-val-test-flac',
    target_root=None,
    target_sr=16000,
    noisy_suffix="_CH0",
    num_workers=None,
    ...
    if target_root is None:
        target_root = os.path.join(script_dir, 'soundai-val-test-wav')
    ...
    conversions = [
        ...
        (
            ...
            os.path.join(target_root, 'test_clean'),
            os.path.join(target_root, 'test_noisy'),
            'test'
        ),
        # val -> val
        (
            ...
            os.path.join(target_root, 'val_clean'),
            os.path.join(target_root, 'val_noisy'),
            'val'
        ),
    ]
    # conversions = [
    ...
    for source_clean_dir, source_noisy_dir, target_clean_dir, target_noisy_dir, dataset_name in conversions:
        ...
        for index, clean_flac_path in enumerate(clean_flac_files):
            ...
            clean_wav_filename = 'clean' + os.path.splitext(clean_filename)[0] + '.wav'
            ...
            if noisy_flac_path and os.path.exists(noisy_flac_path):
                ...
                # 生成噪声WAV路径（仅仅保留index)
                # noisy_filename = 'noisy' + os.path.splitext(clean_filename)[0] + '.wav'
                # noisy_wav_filename = os.path.splitext(noisy_filename)[0] + '.wav'
                noisy_wav_filename = 'noisy' + os.path.splitext(clean_filename)[0] + '.wav'
                noisy_wav_path = os.path.join(target_noisy_dir, noisy_wav_filename)
                
                # 添加到转换任务
                conversion_tasks.append((clean_flac_path, clean_wav_path, target_sr, normalize_volume))
                conversion_tasks.append((noisy_flac_path, noisy_wav_path, target_sr, normalize_volume))
                # 记录映射关系（使用相对路径）
                file_mapping.append({
